...
function Q_bezier(t,j,i,coordinate){
	if(i>=controlPts.length)
			i=0;
	if(j<=0)
	{
		//console.log(controlPts[i].coordinate);
		if(coordinate=='x')
		...
		return ((1-t)*Q_bezier(t,j-1,i, coordinate)) + (t*Q_bezier(t,j-1,i+1, coordinate));
}
...
// ===YOUR CODE STARTS HERE===
	//curvePts = []; // empty array
	//Num_segments= controlPts.length-1;
	for(let i=0; i<=nSegment; i++)
	{ 
		let t = i /nSegment; 
		curvePts.push({'x':Q_bezier(t,controlPts.length-1,0,'x'), 'y':Q_bezier(t,controlPts.length-1,0,'y')}); // use recursive formula to compute this
		if (i!=0) drawLine(curvePts[i-1], curvePts[i]);
		//console.log(curvePts[i]); 
	} 
...
function Q_bspline(t,num,coordinate)
{
	let M = [[-1/6.0,  3/6.0,  -3/6.0,   1/6.0],
		 	[3/6.0,  -6/6.0,   3/6.0,   0],
		 	[-3/6.0,  0,  3/6.0,   0],
			[ 1/6.0,      4/6.0,    1/6.0,  0]];
	let A=[];
	...
	for(i=0;i<4;i++)
	{
		...
		for(j=0;j<4;j++,n++)
		{
			if(n>=controlPts.length)
				n=0;
			if(coordinate=='x')
			{
				sum+= M[i][j]* controlPts[n].x;
				//console.log(A[i]); 
			}
			else
			{
				sum+= M[i][j]* controlPts[n].y;
			}
			//console.log(sum);
		}
		A.push(sum);
...
// ===YOUR CODE STARTS HERE===
	...
	if(!closedCurve)
	{
		for(let k=0; k<controlPts.length-3; k++)
		{
			for(let i=0; i<=nSegment; i++)
			{ 
				let t = i /nSegment; 
				curvePts.push({'x':Q_bspline(t,k,'x'), 'y':Q_bspline(t,k,'y')}); 
				if (i!=0) drawLine(curvePts[i-1], curvePts[i]);
				//console.log(curvePts[i]); 
			}
		} 
	...
	{
		for(let k=0; k<controlPts.length; k++)
		{
			for(let i=0; i<=nSegment; i++)
			{ 
				let t = i /nSegment; 
...
function Q_hermite(t,num,coordinate)
{
	let M = [[2,  -2,  1,   1],
		 	[-3,  3,   -2,   -1],
		 	[0,  0,  1,   0],
...
// ===YOUR CODE STARTS HERE===
	if(!closedCurve)
		{
			for(let k=0; k<controlPts.length-1; k++)
			{
				for(let i=0; i<=nSegment; i++)
				{ 
					let t = i /nSegment; 
					curvePts.push({'x':Q_hermite(t,k,'x'), 'y':Q_hermite(t,k,'y')}); 
					if (i!=0) drawLine(curvePts[i-1], curvePts[i]);
					//console.log(curvePts[i]); 
				}
			} 
