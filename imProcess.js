...
// ===YOUR CODE STARTS HERE===
	let x= (1/sharpness)+4.0;
	let sharpkernel = [[0,-(sharpness),0], [-(sharpness),(x*sharpness),-(sharpness)], [0,-(sharpness),0]];
	filterImage(input, output, sharpkernel);
// ---YOUR CODE ENDS HERE---
}
...
// ===YOUR CODE STARTS HERE===
	...
	for(y=0;y<input.height;y++) {
		for(x=0;x<input.width;x++) {
			let r= (4*((y*input.width)+x))+0;
			let g= (4*((y*input.width)+x))+1;
			let b= (4*((y*input.width)+x))+2;
			let e = bayers[x%4][y%4] * 255;
			let L = (0.3*ip[r]) + (0.59*ip[g]) + (0.11*ip[b]);
			/*op[r] =L > e ? 255 : 0;
			op[g] =L > e ? 255 : 0;
			op[b] =L > e ? 255 : 0;
			*/if(L<=e){
...
// ===YOUR CODE STARTS HERE===
	...
	let den_arr_b = [];
	for(k=0; k<num; k++){
		mimages[k].loadPixels();
		let mimg= mimages[k].pixels;
		for(j=0; j<h; j++){
			for(i=0; i<w; i++){
				den_arr_r[k]+= mimg[(4*((j*w)+i))+0]*mimg[(4*((j*w)+i))+0];
				den_arr_g[k]+= mimg[(4*((j*w)+i))+1]*mimg[(4*((j*w)+i))+1];
				den_arr_b[k]+= mimg[(4*((j*w)+i))+2]*mimg[(4*((j*w)+i))+2];
			}
		}
	}
	console.log(den_arr_r[3]); */
...
// ===YOUR CODE STARTS HERE===
			...
			ip=input.pixels;
			a_r= a_g= a_b =0;
			for(k=0; k<num; k++) { // loop over candidates
				mimages[k].loadPixels();
				let mimg= mimages[k].pixels;
				d_r = d_g = d_b = 0;
				den_r= den_g= den_b=0;
				for(j=0; j<h; j++) {
					for(i=0; i<w; i++) {  // loop over pixels in block
						//update d_r, d_g, d_b;
						/* d_r+= Math.pow(ip[(4*(((y+j)*width)+(x+i)))+0] - mimg[(4*((j*w)+i))+0], 2);
						d_g+= Math.pow(ip[(4*(((y+j)*width)+(x+i)))+1] - mimg[(4*((j*w)+i))+1], 2);
						d_b+= Math.pow(ip[(4*(((y+j)*width)+(x+i)))+2] - mimg[(4*((j*w)+i))+2], 2); */
						d_r+= ip[(4*(((y+j)*width)+(x+i)))+0] * mimg[(4*((j*w)+i))+0];
						den_r+= mimg[(4*((j*w)+i))+0] * mimg[(4*((j*w)+i))+0];

						d_g+= ip[(4*(((y+j)*width)+(x+i)))+1] * mimg[(4*((j*w)+i))+1];
						den_g+= mimg[(4*((j*w)+i))+1] * mimg[(4*((j*w)+i))+1];

						d_b+= ip[(4*(((y+j)*width)+(x+i)))+2] * mimg[(4*((j*w)+i))+2];
						den_b+= mimg[(4*((j*w)+i))+2] * mimg[(4*((j*w)+i))+2];
					}
				}
				//calculate d = d_r + d_g + d_b;
				/* d_r= (-(d_r*d_r))/den_arr_r[k];
				d_g= (-(d_g*d_g))/den_arr_g[k];
				d_b= (-(d_b*d_b))/den_arr_b[k]; */
				d= ((-(d_r*d_r))/den_r) + ((-(d_g*d_g))/den_g) + ((-(d_b*d_b))/den_b);
				//multiply d with random noise between 1 and 2
				r_d= d* (Math.random()+1);
				//keep track of best matches (i.e. smallest d)
				if(min_dist==0 || r_d<min_dist){
					min_dist=r_d;
					best_match = mimages[k];
			...
			/*output.copy(best_match, 0, 0, w, h, x, y, width, height);
			output.loadPixels();
			*/op= output.pixels;
			best_match.loadPixels();
			bm= best_match.pixels;
			console.log(a_r);
			for(j=0; j<h; j++) {
				for(i=0; i<w; i++) {
					op[(4*(((y+j)*width)+(x+i)))+0] = pixelClamp(bm[(4*((j*w)+i))+0]*a_r);
					op[(4*(((y+j)*width)+(x+i)))+1] = pixelClamp(bm[(4*((j*w)+i))+1]*a_g);
					op[(4*(((y+j)*width)+(x+i)))+2] = pixelClamp(bm[(4*((j*w)+i))+2]*a_b);
				}
			}
