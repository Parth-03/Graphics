...
// ===YOUR CODE STARTS HERE===
		...
		let spotlight_vector = ls.position.clone().sub(this.to).normalize(); // 'from - to' vector
		let shadingPoint_vector = ls.position.clone().sub(shadingPoint).normalize(); // 'from - shading point' vector
		
		let cos_angle = spotlight_vector.dot(shadingPoint_vector); // cos(angle between them) =  spot light . shading vector
		let cos_cutoff = Math.cos(this.cutoff * Math.PI / 180);
		if (cos_angle >= cos_cutoff ) // if in the cutoff
		{ 
			
			ls.intensity.multiplyScalar(Math.pow(cos_angle,this.exponent)).multiplyScalar(1/ls.direction.lengthSq());	// quadratic falloff of intensity
		}
		else
		{
